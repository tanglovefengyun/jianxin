...
const {
  ...
  containerProps,
  wrapperProps
} = useVirtualList(memberFilterList, {
  itemWidth: 42,
  overscan: 10
})
...
async function loadData() {
  ...
  const res = await getRoomGroupUserPage(data.roomId, pageInfo.value.size, pageInfo.value.cursor, user.getToken)
  ...
  ContextMenu.showContextMenu({
    ...
    items: [
      ...
      },
      {
        ...
        hidden: isSelf,
        onClick: () => {
          isChatFriend({ uidList: [item.userId] }, user.getToken)
            .then((res) => {
              if (res.code !== StatusCode.SUCCESS) {
    ...
    <div v-if="!isReload" class="h-42px w-500px overflow-hidden" v-bind="wrapperProps">
      <div class="w-500px flex items-center overflow-x-auto" v-bind="containerProps">
        <div
          ...
          @contextmenu="($event: any) => onContextMenu($event, p.data)"
          @click="
          ...
          "
        >
          <CardElImage :default-src="p.data.avatar" fit="cover" error-class="i-solar-user-line-duotone" class="avatar" />
          <span class="g-avatar" />
        </div>
      </div>
