...
async function onHandelRobot(robot: RobotUserVO) {
  ...
  ElMessageBox.confirm('是否添加该 AI ？', {
    ...
    callback: async (action: string) => {
      if (action === 'confirm') {
        // 确认是否为好友
        isChatFriend({ uidList: [userId] }, user.getToken)
          .then(async (fRes) => {
            if (fRes.code !== StatusCode.SUCCESS) return ElMessage.error(fRes.msg || '申请失败，请稍后再试！')
            const user = fRes.data.checkedList.find((p: FriendCheck) => p.uid === userId)
            if (user && user.isFriend) return ElMessage.warning('申请失败，该机器人已添加过！')
            // 开启申请
            ...
            if (res.code !== StatusCode.SUCCESS) return
...
<template>
  <div class="list">
    ...
    <div v-for="p in list" :key="p.userId" class="item" title="开始对话" @click.stop="onHandelRobot(p)">
      <CardElImage v-loading="isLoadRobot === p.userId" class="avatar-icon" :src="BaseUrlImg + p?.avatar" fit="cover" title="点击查看详情" />
      <div>
        <p truncate text-sm>
          {{ p?.nickname || '未填写' }}
        </p>
    ...
    <div v-if="!isReload" class="grid-col-[1/-1] text-center text-mini">
