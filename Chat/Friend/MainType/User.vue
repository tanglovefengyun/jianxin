...
<template>
  ...
  >
    ...
    <div flex gap-4 pb-6 sm:gap-6 border-default-b>
      ...
      /> -->
      <img :src="panelData.data.portrait" class="h-4rem w-4rem flex-shrink-0 overflow-auto object-cover shadow-sm sm:(h-4.8rem w-4.8rem) border-default card-default">
      <div flex flex-col gap-1 py-1>
        <strong truncate text-1.2rem>{{ panelData.data.nickname }}</strong>
    ...
    <div gap-6 pb-6 border-default-b>
      <p truncate text-sm>
        <i mr-3 p-2 :class="panelData.data.sex === 1 ? 'i-tabler:gender-male text-blue' : panelData.data.sex === 2 ? 'i-tabler:gender-female text-pink' : 'i-tabler:gender-transgender text-yellow'" />
        <span class="mr-2 pr-2 border-default-r">
          {{ panelData.data.sex === 1 ? '男' : panelData.data.sex === 2 ? '女' : '未知' }}
        </span>
      ...
      <p mt-6 truncate text-sm>
        ...
        部门：{{ panelData.data.department.name }}
      </p>
      <p mt-6 truncate text-sm>
        <i class="i-tabler:calendar mr-3 p-2" />
        职位：{{ panelData.data.job }}
      </p>
      <!-- <p mt-6 truncate text-sm>
