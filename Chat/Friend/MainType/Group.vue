...
const isGroupOwner = computed(() => data.data.owner_id === user.userInfo.id);
...
<template>
  ...
  >
    ...
    <div flex gap-4 pb-6 sm:gap-6 border-default-b>
      <img :src="data.data.portrait" alt="" class="h-4rem w-4rem flex-shrink-0 overflow-auto object-cover shadow-sm sm:(h-4.8rem w-4.8rem) border-default card-default">
      <div flex flex-col gap-1 py-1>
        <strong h-1.2em truncate text-1.2rem leading-1em>{{ data.data.name }}</strong>
        <p mt-a truncate text-xs text-color>
          人数：
          <span class="content" :title="getCreateTime">
            {{ data.data.members_count || 0 }}人
          </span>
        </p>
        <p truncate text-mini>
          群号：
          <BtnCopyText icon="i-solar:copy-bold-duotone" :text="data.data.id" class="inline">
            {{ data.data.id }}
          </BtnCopyText>
        </p>
      </div>
    ...
    <div gap-6 truncate pb-6 border-default-b>
      ...
      <p class="line">
        ...
        群成员：<span v-if="room.hotFlag !== isTrue.TRUE">( {{ data.data.members_count || 0 }}人 )</span>
    ...
    <div class="mx-a">
      ...
      <BtnElButton
        ...
        type="primary"
        @click="chat.toContactSendMsg('group', data.data)"
      >
        发送消息&ensp;
      </BtnElButton>
    </div>
