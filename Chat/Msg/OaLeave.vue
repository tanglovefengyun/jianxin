...
async function getLeaveDetails() {
  ...
  const res = await getOaLeaveDetailApi({ leave_request_id: props.data.content.leave_request_id }, user.getToken)
...
async function getAgree(status: number) {
  if (status === 1) {
    try {
      await getOaLeaveApprovalApi({ leave_request_id: props.data.content.leave_request_id, status, remark: '' }, user.getToken)
    ...
    ElMessageBox.prompt('请输入拒绝原因: ', '拒绝', {
      ...
      .then(async ({ value }) => {
        await getOaLeaveApprovalApi({ leave_request_id: props.data.content.leave_request_id, status, remark: value }, user.getToken)
    ...
    <template #body>
      <div class="msg-popper group notice text-left text-0.9rem" ctx-name="content">
        <div ctx-name="content" mb-2 pb-2 text-left border-default-b text-small dark:text-gray-300>
          <i ctx-name="content" class="i-solar:verified-check-bold mr-1 p-2 text-[--el-color-warning] font-500 group-hover:animate-pulse" /> OA审批
        ...
        <div ctx-name="content" class="msg-wrap max-w-20em min-w-14em leading-1.6em text-color">
          {{ data.content.nickname }}提交的{{ getLeaveType(data.content.leave_type) }}申请<br />
          <small>请假类型: {{ getLeaveType(data.content.leave_type) }}</small
          ><br />
          <small>开始时间: {{ data.content.start_time }}</small
          ><br />
          <small>结束时间: {{ data.content.end_time }}</small
          ><br />
          <small>请假事由: {{ data.content.reason }}</small>
        </div>
        <BtnElButton
      ...
      <!-- 分离后的抽屉组件 -->
      <dialogLeaveDrawer v-model:drawer="drawer" :leave-data="leaveData" :loading="btnLoading" @submit="getAgree" type="leave" />
    </template>
  </ChatMsgTemplate>
</template>
