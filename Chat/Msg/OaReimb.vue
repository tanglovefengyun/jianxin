...
async function getLeaveDetails() {
  ...
  const res = await getOaReimbursementDetailApi({ reimbursement_request_id: props.data.content.reimbursement_request_id }, user.getToken)
...
async function getAgree(status: number) {
  if (status === 1) {
    try {
      await getOaReimbursementApprovalApi({ reimbursement_request_id: props.data.content.reimbursement_request_id, status, remark: '' }, user.getToken)
    ...
    ElMessageBox.prompt('请输入拒绝原因: ', '拒绝', {
      ...
      .then(async ({ value }) => {
        await getOaReimbursementApprovalApi({ reimbursement_request_id: props.data.content.reimbursement_request_id, status, remark: value }, user.getToken)
...
const leaveTypes = ['', '部门活动经费', '办公费', '交通费', '招待费', '差旅', '其他支出']
    ...
    <template #body>
      <div class="msg-popper group notice text-left text-0.9rem" ctx-name="content">
        <div ctx-name="content" mb-2 pb-2 text-left border-default-b text-small dark:text-gray-300>
          <i ctx-name="content" class="i-solar:verified-check-bold mr-1 p-2 text-[--el-color-warning] font-500 group-hover:animate-pulse" /> 报销申请
        ...
        <div ctx-name="content" class="msg-wrap max-w-20em min-w-14em leading-1.6em text-color">
          {{ data.content.nickname }}提交的报销申请<br />
          <small>报销类型: {{ getLeaveType(data.content.reimbursement_type) }}</small
          ><br />
          <small>费用日期: {{ data.content.date }}</small
          ><br />
          <small>报销金额: {{ data.content.money }}</small
          ><br />
          <small>费用说明: {{ data.content.reason }}</small>
        </div>
        <BtnElButton
      ...
      <!-- 分离后的抽屉组件 -->
      <dialogLeaveDrawer v-model:drawer="drawer" :leave-data="leaveData" :loading="btnLoading" @submit="getAgree" type="reimb" />
    </template>
  </ChatMsgTemplate>
</template>
