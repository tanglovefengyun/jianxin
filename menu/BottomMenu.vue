...
const unWatchDebounced = watchDebounced(
...
<template>
  <div class="relative z-998 grid grid-cols-4 select-none justify-center border-0 border-t-1px bg-white shadow-md border-default dark:bg-dark-8">
    <div
      v-for="p in menuList"
      :key="p.path"
      :index="p.path"
      ...
      @click.stop="
        () => {
          if (p.path) activeMenu = p.path
        }
      "
    >
      <el-badge v-if="!p?.children?.length" :value="p?.tipValue || 0" :hidden="!p?.tipValue" :max="99" :is-dot="p.isDot">
        <i class="p-3" :class="route.path === p.path ? p.activeIcon : p.icon" />
        <span mt-2 block select-none text-center text-3>{{ p.title }}</span>
      </el-badge>
      <el-popover v-else :width="30" transition="popper-fade-up" :offset="25" trigger="click">
        <template #reference>
          <el-badge
            :value="p.tipValue"
            :hidden="!p.tipValue"
        ...
        <template #default>
          <ul class="grid cols-1 gap-3">
            <el-badge
              ...
              :max="99"
              :is-dot="item.isDot"
              class="flex-row-c-c cursor-pointer gap-6 px-2 py-1"
              :index="p.path"
