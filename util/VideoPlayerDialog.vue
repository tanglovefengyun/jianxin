  ...
  onMove: (position) => {
    ...
    if (dragRef.value) {
      if (newX <= 50 || newY <= 50) {
        ...
        newX >= innerWidth - (dragRefStyle.value.maxWidth || dragRef.value.offsetWidth) ||
        newY >= innerHeight - (dragRefStyle.value.maxHeight || dragRef.value.offsetHeight)
      ) {
...
const videoSize = computed(() => {
  ...
  if (videoInfo.value?.thumbWidth !== undefined && videoInfo.value?.thumbHeight !== undefined && globalThis) {
    ...
    if (setting.isMobileSize) {
      ...
      const { width, height } = computedImgScaleSize(videoInfo.value.thumbWidth, videoInfo.value.thumbHeight, {
        maxWidth: Math.floor(windowWidth * 0.6),
...
<template>
  <Transition mode="out-in" enter-active-class="animate-zoom-in-dailog" leave-active-class="animate-zoom-out-dailog">
    <div
      v-show="show"
      title="视频播放器"
      class="video-player-dialog fixed left-0 top-0 z-9999 h-full w-full flex-row-c-c"
      ...
      >
        <div class="menu absolute left-0 top-0 z-1 w-full flex flex items-center p-2">
          ...
          <div class="ml-a flex-row-c-c gap-4 px-4 py-2 transition-opacity card-default-br sm:(op-0 group-hover:op-100)">
            <div @click.stop="saveVideoLocal(videoInfo.url)">
              <i class="i-solar:download-minimalistic-linear" title="另存为" p-2.4 filter-drop-shadow-lg btn-info />
            </div>
            <div @click.stop="toggleFullscreen">
              <i :class="isFullscreen ? 'i-tabler:minimize' : 'i-tabler:maximize'" title="全屏" p-2.4 filter-drop-shadow-lg btn-info />
            </div>
