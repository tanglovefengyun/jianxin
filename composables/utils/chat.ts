 ...
 * @returns 消息内容
 */
export function resolveMsgReplyText(msg: any): string | undefined | null {
  if (msg.type === 'voice') {
    return `[语音] ${msg.voice_duration}"`;
  }
  ...
  else if (msg.type === 'group_notice') {
    return `[群公告]`;
  }
  else if (msg.type === 'oa_leave') {
    return `[OA审批]`;
  }
  else if (msg.type === 'oa_todo') {
    return `[待办事项]`;
  }
  else if (msg.type === 'oa_reimbursement') {
    return `[报销申请]`;
  }
  else if (msg.type === 'oa_overtime') {
    return `[加班审批]`;
  }
  // else if (msg.message.type === MessageType.RTC) {
  //   const _msg = msg as ChatMessageVO<RtcLiteBodyMsgVO>;
